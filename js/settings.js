...
(function () {
  ...
  function fire(key, val) {
    ...
    if (String(key).indexOf("svc-pet") === 0) window.dispatchEvent(new Event("tw:pet"));
  }
  ...
  panel.innerHTML =
    ...
    settingRow("THEME", '<span class="seg" data-pref="svc-theme">' +
      '<button data-val="dark">DARK</button><button data-val="light">LIGHT</button>' +
      '<button data-val="system">SYSTEM</button></span>') +
    settingRow("ACCENT", '<span class="seg accent-seg" data-pref="svc-accent">' +
    ...
    settingRow("MODE", '<span class="seg" data-petpref="svc-pet">' +
    ...
    settingRow("NAP", petToggle("svc-pet-nap")) +
    settingRow("FLEE", petToggle("svc-pet-flee")) +
    settingRow("READ", petToggle("svc-pet-read")) +
    settingRow("TRICKS", petToggle("svc-pet-tricks")) +
    settingRow("SPEECH", petToggle("svc-pet-speech")) +
    "</div></div>";
  ...
  function petDefaults(key) {
    return { "svc-pet": "float", "svc-pet-nap": "on", "svc-pet-flee": "on",
      "svc-pet-read": "on", "svc-pet-tricks": "on", "svc-pet-speech": "off" }[key];
  }
  ...
  function refreshSeg() {
    ...
    for (var i = 0; i < segs.length; i++) {
      ...
      var cur;
      if (key === "svc-theme") cur = TW.prefs.get("svc-theme", "system", ["dark", "light", "system"]);
      else cur = TW.prefs.get("svc-accent", "0", ["1", "2", "3", "4", "5"]);
  ...
  function refreshPet() {
    ...
    document.getElementById("pet-size").value = TW.prefs.get("svc-pet-size", "28");
    document.getElementById("pet-opacity").value = TW.prefs.get("svc-pet-opacity", "70");
  }
  ...
  panel.addEventListener("click", function (e) {
    ...
    if (key === "svc-theme") {
      if (val === "system") { TW.prefs.remove("svc-theme"); }
      else { TW.prefs.set("svc-theme", val); }
      applyTheme(val === "system" ? null : val);
      refreshSeg();
      return;
    }
    if (key === "svc-accent") {
      if (val === "0") { TW.prefs.remove("svc-accent"); }
      else { TW.prefs.set("svc-accent", val); }
